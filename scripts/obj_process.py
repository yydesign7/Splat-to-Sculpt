#!/usr/bin/env python3
"""
OBJ/GLB + PNG Processing Pipeline
Integrates functionality from 1.py (texture extraction, metadata, YOLO+SAM, COCO)
and 2.py (UV completion, texture mapping, rendering).

Usage:
  python3 obj_process.py \
    --input <model_file_path> \
...
# ===================== 1. Layered Texture Extraction (from 1.py) =====================
def extract_textures(mesh, output_dir, model_name):
    """Extract layered textures from the 3D model (diffuse, normal, metal_rough, UV vis)."""
    textures = {}
    ...
    try:
        if hasattr(mesh.visual, "material") and hasattr(mesh.visual.material, "baseColorTexture"):
            if mesh.visual.material.baseColorTexture is not None:
                ...
                path = os.path.join(output_dir, f"{model_name}_diffuse.png")
    ...
    try:
        if hasattr(mesh.visual, "material") and hasattr(mesh.visual.material, "normalTexture"):
            if mesh.visual.material.normalTexture is not None:
                ...
                path = os.path.join(output_dir, f"{model_name}_normal.png")
    ...
    try:
        if hasattr(mesh.visual, "material") and hasattr(mesh.visual.material, "metallicRoughnessTexture"):
            if mesh.visual.material.metallicRoughnessTexture is not None:
                ...
                path = os.path.join(output_dir, f"{model_name}_metal_rough.png")
                Image.fromarray(metal).save(path)
                textures["metal_rough"] = path
    except Exception:
        pass
    ...
    try:
        if hasattr(mesh.visual, "uv") and mesh.visual.uv is not None:
            ...
            path = os.path.join(output_dir, f"{model_name}_uv_vis.png")
            Image.fromarray(uv_map).save(path)
            textures["uv_visual"] = path
    except Exception:
        pass
...
# ===================== 2. Metadata Export (from 1.py) =====================
def export_metadata(mesh, texture_paths, output_dir, model_name, file_type):
    """Export complete metadata JSON: scale, orientation, materials, UV, vertex/face counts."""
    has_uv = False
    uv_shape = None
    try:
        if hasattr(mesh.visual, "uv") and mesh.visual.uv is not None:
            has_uv = True
            uv_shape = list(mesh.visual.uv.shape)
    except Exception:
        pass
    ...
    metadata = {
        ...
        "scale_metadata": {
            "scale_factor": 1.0,
            "unit": "meter",
            "bounding_box_size": [float(x) for x in mesh.extents],
        },
...
# ===================== 3. Render Model (from 1.py, with EGL) =====================
def render_model(mesh, output_path, image_size=(1024, 1024)):
    """Render a high-quality 2D image.
    Primary: pyrender + EGL offscreen rendering.
    Fallback 1: pyrender with ColorVisuals (avoids glGenTextures bug).
    Fallback 2: matplotlib (pure software, no EGL needed).
    """
    ...
    try:
        ...
        # Try to get face colors from the visual
        try:
            colors = mesh.visual.face_colors
...
# ===================== 4. YOLOv8 + SAM (from 1.py, optional) =====================
...
def complete_uv(mesh):
    ...
    # Spherical UV projection (from 2.py)
...
# ===================== 6. Apply Texture to Model (from 2.py) =====================
...
# ===================== Main Pipeline =====================
def process_pipeline(obj_path, texture_path, output_dir, no_glb=False):
    """Run the full OBJ + PNG processing pipeline."""
    ...
    # Step 2: Extract layered textures (from original model, if any)
    textures_dir = os.path.join(output_dir, "textures")
    os.makedirs(textures_dir, exist_ok=True)
    texture_paths = extract_textures(mesh, textures_dir, model_name)

    # Step 3: Export metadata
    metadata_path = export_metadata(mesh, texture_paths, output_dir, model_name, "obj")

    # Step 4: Complete UV (reuse existing or compute spherical)
    mesh, uv, had_existing_uv = complete_uv(mesh)

    # Step 5: Apply the provided PNG texture
    mesh = apply_texture(mesh, texture_path, uv)

    # Step 6: Save the new textured model
    new_model_path = os.path.join(output_dir, f"{model_name}_textured.obj")
    mesh.export(new_model_path)

    # Also save as GLB for better preview compatibility (unless --no-glb)
    new_glb_path = None
    if not no_glb:
    ...
    # Step 7: Render the textured model
    render_path = os.path.join(output_dir, f"{model_name}_render.png")
    try:
        render_model(mesh, render_path)
    except Exception as e:
    ...
    result = {
        ...
        "uv_source": "existing" if had_existing_uv else "spherical_projection",
...
def main():
    parser = argparse.ArgumentParser(description="OBJ + PNG Processing Pipeline")
    parser.add_argument("--input", required=False, help="Path to input model file (OBJ/GLB)")
    ...
    parser.add_argument("--no-glb", action="store_true", help="Skip GLB generation")
    args = parser.parse_args()

    obj_path = args.input or args.obj
    if not obj_path:
        parser.error("Either --input or --obj is required")

    try:
        result = process_pipeline(obj_path, args.texture, args.output_dir, no_glb=args.no_glb)
